...
def load_model_covered_match_ids():
    ...
    model_files = [
        LIVE_DIR / "v5_live_predictions_core.csv",
    ]

    # Auto-discover the separate core/Understat live file
    # containing Premier League / Bundesliga predictions.
    for candidate in LIVE_DIR.glob("*v5*prediction*.csv"):

        if candidate.name == "v5_live_predictions_core.csv":
            continue
        ...
        if "league" not in probe.columns:
            continue
        ...
        if (
            "Premier League" in leagues
            or
            "Bundesliga" in leagues
        ):
            model_files.append(
                candidate
            )
